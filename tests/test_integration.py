...
class TestSwiftestIntegration(unittest.TestCase):
    ...
    def test_conservation(self):
        ...
        self.assertLess(
            np.abs(L_slope),
            L_slope_limit,
            msg=f"Angular Momentum Error of {L_slope:.2e}/{sim.TU_name} higher than threshold value of {L_slope_limit:.2e}/{sim.TU_name}",
        )
        self.assertLess(
            np.abs(E_slope),
            E_slope_limit,
            msg=f"Energy Error of {E_slope:.2e}/{sim.TU_name} higher than threshold value of {E_slope_limit:.2e}/{sim.TU_name}",
        )
        self.assertLess(
            np.abs(GM_final), GM_limit, msg=f"Mass Error of {GM_final:.2e} higher than threshold value of {GM_limit:.2e}"
        )
        return
    ...
    def test_gr(self):
        ...
        sim = swiftest.Simulation(
            simdir=self.simdir, tstop=5000.0, dt=0.01, tstep_out=tstep_out, dump_cadence=0, general_relativity=True
        )
        ...
        obj = Horizons(id="1", location="@sun", epochs={"start": start_date, "stop": stop_date, "step": f"{tstep_out}y"})
        el = obj.elements()
        ...
        for i in integrators:
            ...
            self.assertLess(
                np.abs(dvarpi_err),
                dvarpi_limit,
                msg=f'{dvarpi_err:.2e} /{sim.TU_name} is higher than threshold value of {dvarpi_limit:.2e} "/{sim.TU_name}',
            )
