...
contains
   ...
   module subroutine helio_kick_yarkovsky_pl(self, nbody_system, param)
      ...
      associate(pl => self)
         do i=1, pl%nbody
               if (pl%lmask(i)) then
                  call swiftest_yarkovsky_getacch_pl_one(lag_angle_constants, pl%mu(i), pl%mass(i), pl%radius(i), pl%rh(:, i), pl%vb(:, i), pl%rot(:, i), pl%a(i), 
                                                         pl%emissivity(i), pl%gamma(i), pl%albedo(i), pl%rot_k(i), param%L_SUN_sys, param%inv_c2, a_yark)
                  pl%ah(:, i) = pl%ah(:, i) + a_yark(:)
               end if 
         end do
      return
   end subroutine helio_kick_yarkovsky_pl
