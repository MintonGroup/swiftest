...
contains
   ...
   module function netcdf_get_old_t_final_system(self, param) result(old_t_final)
      ...
      associate (nc => param%system_history%nc)
         ...
         if (param%lenergy) then
            ...
            call check( nf90_get_var(nc%id, nc%Lspin_varid, self%Lspin_orig(:), start=[1,1], count=[NDIM,1]), "netcdf_get_old_t_final_system Lspin_varid" )
   ...
   module subroutine netcdf_initialize_output(self, param)
      ...
      associate(nc => self)
         ...
         if (param%lenergy) then
            ...
            call check( nf90_def_var(nc%id, nc%Lspin_varname, nc%out_type, [nc%space_dimid, nc%time_dimid], nc%Lspin_varid), "netcdf_initialize_output nf90_def_var Lspin_varid"  )
   ...
   module subroutine netcdf_read_hdr_system(self, nc, param) 
      ...
      if (param%lenergy) then
         ...
         status = nf90_inq_varid(nc%id, nc%Lspin_varname, nc%Lspin_varid)
         if (status == nf90_noerr) call check( nf90_get_var(nc%id, nc%Lspin_varid,     self%Lspin(:),    start=[1,tslot], count=[NDIM,1]), "netcdf_read_hdr_system nf90_getvar Lspin_varid"  )
         status = nf90_inq_varid(nc%id, nc%L_escape_varname, nc%L_escape_varid)
   ...
   module subroutine netcdf_write_hdr_system(self, nc, param) 
      ...
      if (param%lenergy) then
         ...
         call check( nf90_put_var(nc%id, nc%Lspin_varid, self%Lspin(:), start=[1,tslot], count=[NDIM,1]), "netcdf_write_hdr_system nf90_put_var Lspin_varid"  )
