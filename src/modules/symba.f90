...
!! Interfaces for all helio particle methods that are implemented in separate submodules 
   ...
   module subroutine symba_casedisruption (t, dt, index_enc, nmergeadd, nmergesub, mergeadd_list, mergesub_list, eoffset, vbs, & 
      symba_plA, nplplenc, plplenc_list, fragmax, mres, rres, m1, m2, rad1, rad2, x1, x2, v1, v2, param)
      implicit none
   ...
   module subroutine symba_caseresolve (t, dt, index_enc, nmergeadd, nmergesub, mergeadd_list, mergesub_list, &
      ...
      implicit none
      integer(I4B), intent(in)       :: index_enc, param%nplmax, param%ntpmax
   ...
   module subroutine symba_discard_pl(t, npl, param%nplmax, nsp, symba_plA, rmin, rmax, param%rmaxu, qmin, qmin_coord,   &
      qmin_alo, qmin_ahi, param%j2rp2, param%j4rp4, eoffset)
      implicit none
      integer(I4B), intent(in)   :: param%nplmax
      integer(I4B), intent(inout) :: npl, nsp
      real(DP), intent(in)   :: t, rmin, rmax, param%rmaxu, qmin, qmin_alo, qmin_ahi, param%j2rp2, param%j4rp4
      real(DP), intent(inout)   :: eoffset
      character(*), intent(in)   :: qmin_coord
      type(symba_pl), intent(inout)   :: symba_plA
   end subroutine symba_discard_pl

   module subroutine symba_discard_sun_pl(t, npl, msys, swiftest_plA, rmin, rmax, param%rmaxu, ldiscards)
      implicit none
      logical , intent(inout) :: ldiscards
      integer(I4B), intent(in)   :: npl
      real(DP), intent(in)   :: t, msys, rmin, rmax, param%rmaxu
      type(swiftest_pl), intent(inout)   :: swiftest_plA
   end subroutine symba_discard_sun_pl

   module subroutine symba_discard_tp(t, npl, ntp, nsp, symba_plA, symba_tpA, dt, &
      rmin, rmax, param%rmaxu, qmin, qmin_coord, qmin_alo, qmin_ahi, lclose, lrhill_present)
      implicit none
      logical , intent(in)   :: lclose, lrhill_present
      integer(I4B), intent(in)   :: npl
      integer(I4B), intent(inout) :: ntp, nsp
      real(DP), intent(in)   :: t, dt, rmin, rmax, param%rmaxu, qmin, qmin_alo, qmin_ahi
      character(*), intent(in)   :: qmin_coord
      type(symba_pl), intent(inout)   :: symba_plA
      type(symba_tp), intent(inout)   :: symba_tpA
   end subroutine symba_discard_tp

   module subroutine symba_energy(npl, param%nplmax, swiftest_plA, param%j2rp2, param%j4rp4, ke, pe, te, htot)
      implicit none
      integer(I4B), intent(in)      :: npl, param%nplmax
      real(DP), intent(in)       :: param%j2rp2, param%j4rp4
   ...
   module subroutine symba_fragmentation(t, dt, index_enc, nmergeadd, nmergesub, mergeadd_list, &
      ...
      param%nplmax, param%ntpmax, fragmax)
      implicit none
      integer(I4B), intent(in)       :: index_enc, param%nplmax, param%ntpmax
   ...
   module subroutine symba_getacch(lextra_force, t, npl, nplm, param%nplmax, symba_plA, param%j2rp2, param%j4rp4, nplplenc, &
      plplenc_list)
      implicit none
      logical , intent(in)        :: lextra_force
      integer(I4B), intent(in)        :: npl, nplm, param%nplmax, nplplenc
      real(DP), intent(in)        :: t, param%j2rp2, param%j4rp4
      type(symba_pl), intent(inout)      :: symba_plA
      type(symba_plplenc), intent(in)      :: plplenc_list
   end subroutine symba_getacch

   module subroutine symba_getacch_tp(lextra_force, t, npl, nplm, param%nplmax, ntp, param%ntpmax, symba_plA, symba_tpA, &
      xh, param%j2rp2, param%j4rp4,  &
      npltpenc, pltpenc_list)
      implicit none
      logical , intent(in)        :: lextra_force
      integer(I4B), intent(in)        :: npl, nplm, param%nplmax, ntp, param%ntpmax, npltpenc
      real(DP), intent(in)        :: t, param%j2rp2, param%j4rp4
   ...
   module subroutine symba_getacch_eucl(lextra_force, t, npl, nplm, param%nplmax, symba_plA, param%j2rp2, param%j4rp4, nplplenc, &
      plplenc_list, num_plpl_comparisons, k_plpl)
      implicit none
      logical , intent(in)        :: lextra_force
      integer(I4B), intent(in)        :: npl, nplm, param%nplmax, nplplenc, num_plpl_comparisons
      real(DP), intent(in)        :: t, param%j2rp2, param%j4rp4
      type(symba_pl), intent(inout)      :: symba_plA
      type(symba_plplenc), intent(in)      :: plplenc_list
   ...
   module subroutine symba_getacch_tp_eucl(lextra_force, t, npl, nplm, param%nplmax, ntp, param%ntpmax, symba_plA, symba_tpA, &
      xh, param%j2rp2, param%j4rp4, npltpenc, pltpenc_list, num_pltp_comparisons, k_pltp)
      implicit none
      logical , intent(in)        :: lextra_force
      integer(I4B), intent(in)        :: npl, nplm, param%nplmax, ntp, param%ntpmax, npltpenc, num_pltp_comparisons
      real(DP), intent(in)        :: t, param%j2rp2, param%j4rp4
      real(DP), dimension(npl, NDIM), intent(in)   :: xh
      type(symba_pl), intent(inout)      :: symba_plA
      type(symba_tp), intent(inout)      :: symba_tpA
   ...
   module subroutine symba_helio_getacch(lflag, lextra_force, t, npl, nplm, param%nplmax, helio_plA, param%j2rp2, param%j4rp4)
      implicit none
   ...
   module subroutine symba_rearray(t, npl, ntp, nsppl, nsptp, symba_plA, symba_tpA, nmergeadd, &
      mergeadd_list, discard_plA, discard_tpA, param%nplmax, param%j2rp2, param%j4rp4)
      implicit none
      integer(I4B), intent(inout)        :: npl, ntp, nsppl, nsptp, nmergeadd, param%nplmax !change to fragadd
      real(DP), intent(in)          :: t, param%j2rp2, param%j4rp4
      type(symba_pl), intent(inout)      :: symba_plA
      type(symba_tp), intent(inout)      :: symba_tpA
      type(swiftest_tp), intent(inout)      :: discard_tpA
   ...
   module subroutine symba_set_initial_conditions(symba_plA, symba_tpA, param)
      implicit none
   ...
   module subroutine symba_step_eucl(lfirst, lextra_force, lclose, t, npl, param%nplmax, ntp, param%ntpmax, symba_plA, symba_tpA, param%j2rp2, param%j4rp4,&
      ...
      integer(I4B), intent(in)    :: npl, param%nplmax, ntp, param%ntpmax
      integer(I4B), intent(inout)   :: nplplenc, npltpenc, nmergeadd, nmergesub
      real(DP), intent(in)      :: t, param%j2rp2, param%j4rp4, dt, mtiny
      real(DP), intent(inout)     :: eoffset
      character(*), intent(in)    :: encounter_file, out_type
      type(symba_pl), intent(inout)   :: symba_plA
      type(symba_tp), intent(inout)   :: symba_tpA
   ...
   module subroutine symba_step(lfirst, lextra_force, lclose, t, npl, param%nplmax, ntp, param%ntpmax, symba_plA, &
      symba_tpA, param%j2rp2, param%j4rp4, dt, nplplenc, npltpenc, plplenc_list, pltpenc_list, nmergeadd, &
   ...
   module subroutine symba_step_helio(lfirst, lextra_force, t, npl, nplm, param%nplmax, ntp, param%ntpmax, helio_plA, helio_tpA, param%j2rp2,   &
      param%j4rp4, dt)
      implicit none
      logical , intent(in)   :: lextra_force
      logical , intent(inout)   :: lfirst
      integer(I4B), intent(in)   :: npl, nplm, param%nplmax, ntp, param%ntpmax
   ...
   module subroutine symba_step_interp_eucl(lextra_force, lclose, t, npl, nplm, param%nplmax, ntp, param%ntpmax, symba_plA, symba_tpA, param%j2rp2,&
      param%j4rp4, dt, eoffset, mtiny, nplplenc, npltpenc, plplenc_list, pltpenc_list, nmergeadd, nmergesub, mergeadd_list,&
      mergesub_list, encounter_file, out_type, num_plpl_comparisons, k_plpl, num_pltp_comparisons, k_pltp)
      implicit none
      logical , intent(in)    :: lextra_force, lclose
      integer(I4B), intent(in)    :: npl, nplm, param%nplmax, ntp, param%ntpmax, nplplenc, npltpenc, num_pltp_comparisons
   ...
   module subroutine symba_step_interp(lextra_force, lclose, t, npl, nplm, param%nplmax, ntp, param%ntpmax, symba_plA, symba_tpA, param%j2rp2,   &
      param%j4rp4, dt, eoffset, mtiny, nplplenc, npltpenc, plplenc_list, pltpenc_list, nmergeadd, nmergesub, mergeadd_list,   &
      mergesub_list, encounter_file, out_type, fragmax)
      implicit none
      logical , intent(in)    :: lextra_force, lclose
      integer(I4B), intent(in)    :: npl, nplm, param%nplmax, ntp, param%ntpmax, nplplenc, npltpenc
   ...
   recursive module subroutine symba_step_recur(lclose, t, ireci, npl, nplm, ntp, symba_plA, symba_tpA, dt0, eoffset, nplplenc, &
      ...
      out_type, param%nplmax, param%ntpmax, fragmax)
      implicit none
      logical , intent(in)    :: lclose
      integer(I4B), intent(in)    :: ireci, npl, nplm, ntp, nplplenc, npltpenc, param%nplmax, param%ntpmax, fragmax
      integer(I4B), intent(inout)   :: nmergeadd, nmergesub
