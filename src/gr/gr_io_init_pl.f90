...
!    File      : none
!
!  Invocation  : CALL gr_io_init_pl(inplfile, in_type, lclose, lrhill_present, npl, swifter_pl1P, relativity, c2)
!
!  Notes       : Adapted from Martin Duncan's Swift routine gr_io_init_pl.f
!                Includes code to convert input velocities into pseudovelocities for relativistic effects, 
!                based on Saha & Tremaine (1994)
!
!**********************************************************************************************************************************
SUBROUTINE gr_io_init_pl(inplfile, in_type, lclose, lrhill_present, npl, swifter_pl1P, c2)

! Modules
     USE module_parameters
     USE module_swifter
     USE module_fxdr
     USE module_interfaces, EXCEPT_THIS_ONE => gr_io_init_pl
     IMPLICIT NONE
...
! Executable code
     ...
     DO i = 2, npl
          swifter_plP => swifter_plP%nextP
          etaj = etajm1 + swifter_pl1P%mass
          mu = msun * etaj/etajm1
          etajm1 = etaj
          CALL gr_vel2pseudovel(swifter_plP%xh, swifter_plP%vh, mu, c2, swifter_plP%vh)
     END DO

     RETURN

END SUBROUTINE gr_io_init_pl
!**********************************************************************************************************************************
!
