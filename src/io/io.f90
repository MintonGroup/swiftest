...
contains
   ...
   module subroutine io_param_writer(self, unit, iotype, v_list, iostat, iomsg) 
      ...
      associate(param => self)
         ...
         write(param_name, Afmt) "DISCARD_OUT"; write(param_value, Lfmt) param%discard_out; write(unit, Afmt, err = 667, iomsg = iomsg) adjustl(param_name), adjustl(param_value)
         ...
         if (param%lenergy) write(param_name, Afmt) "ENERGY_OUT"; write(param_value, Lfmt)  param%energy_out; write(unit, Afmt, err = 667, iomsg = iomsg) adjustl(param_name), adjustl(param_value)
