...
!    File      : none
!
!  Invocation  : CALL drift_vec_dan(mu, x0, v0, dt0, iflag)
!
...
! executable code
     ...
     if (alpha > 0.0_DP) then
          ...
          if ((esq < e2max) .and. (dm*dm < dm2max) .and. (esq*dm*dm < e2dm2max)) then
               call drift_vec_kepmd(dm, es, ec, xkep, s, c)
...
! dek - check with hal and/or martin about this
     ...
     call drift_vec_kepu(dt, r0, mu, alpha, u, fp, c1, c2, c3, iflag)
     if (iflag == 0) then
          f = 1.0_DP - mu/r0*c2
          g = dt - mu*c3
          fdot = -mu/(fp*r0)*c1
