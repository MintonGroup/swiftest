submodule (drift_vec) s_drift_vec_one
contains

    module procedure drift_vec_one
    ...
    call drift_vec_dan(mu, x(:), v(:), dt, iflag)
    if (iflag /= 0) then
         dttmp = 0.1_DP * dt
         do i = 1, 10
              call drift_vec_dan(mu, x(:), v(:), dttmp, iflag)
              if (iflag /= 0) return
         end do
    end if

    return
    end procedure drift_vec_one

end submodule s_drift_vec_one