...
! Executable code
   ...
   if ((sval < 1._DP).and.(dt * TU2S > DTMIN)) then
      ...
      stepfail = .true.
      dtnew = 0.9_DP * sval * dt  
   !else if (sval > 1._DP) then
   !   dtnew =  sval * dt
   !else
   !   dtnew = DTMIN
      return
   ...
   !where (Ev2(:) > 0.0_DP) 
   !   sv2(:) = (TOL / (2 * Ev2(:)))**(0.25_DP)
   !elsewhere
   !   sv2(:) = huge(1._DP)
   !end where 
!
!   where (EGm(:) > 0.0_DP) 
!      sGm(:) = (TOL / (2 * EGm(:)))**(0.25_DP)
!   elsewhere
!      sGm(:) = huge(1._DP)
!   end where 

   dtnew = max(DTMIN,sval * dt)
   ...
   !write(*,*) maxval(ring%r_pdisk(:)) * DU2CM,maxval(ring%vrel_pdisk) * DU2CM / TU2S
   !read(*,*)
