...
! Executable code
   ...
   r_hstar = R_Planet / (2 * ring%r_pdisk) * (2 *ring%Gm_pdisk /(3._DP * GM_Planet))**(1._DP/3._DP)
   do i = 1, ring%N 
      if (ring%Gsigma(i) <= sigsmall) then
         ring%nu(i) = 0.0_DP
      else
         if (r_hstar < 0.5_DP) then
            sigma_r = 2 * ring%r_pdisk * ring%w(i)
         else
            sigma_r = sqrt(ring%Gm_pdisk / ring%r_pdisk)
         end if
         ...
         if (Q >= 2._DP) then
             ...
             nu_trans = 13 * r_hstar**5 * ring%Gsigma(i)**2 / ring%w(i)**3
             nu_grav = nu_trans
         end if
